...
Below are the four fundamental pillars of my research, unified under the UHRT framework:
...
**Summary:**
This framework proposes an integer-based symbolic logic for neurosymbolic reasoning, acting as a candidate model for the underlying logic of "sparse circuits" in AI. It moves reasoning beyond floating-point vector addition toward a ratio-based balance. [cite_start]All logic uses exact rational arithmetic (`fractions` library) and normalization via the Greatest Common Divisor (GCD).
...
* Provided empirical proof that the failure in generative fine-tuning with DialoGPT-small was due to loss imbalance, where the Language Model Loss ($\approx 4.68$) dominated and essentially ignored the Bipolar Loss ($\approx 0.12$)[cite: 10, 54].
